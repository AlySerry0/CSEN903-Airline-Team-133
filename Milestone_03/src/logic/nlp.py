...
from src.logic.prompts import INTENT_SYSTEM_PROMPT, ENTITY_SYSTEM_PROMPT
import json
import os
...
cm = ConfigManager()
token = cm.get("GITHUB_TOKEN")

if not token:
    # Fallback or error handling if needed, though for now we might just warn
    print("Warning: GITHUB_TOKEN not found in configuration.")

endpoint = "https://models.inference.ai.azure.com"
client = OpenAI(base_url=endpoint, api_key=token or "token")


# Create a map of city names -> airport codes
# Go up 3 levels from src/logic/nlp.py -> src/logic -> src -> root -> resources
RESOURCES_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "resources")
airline_df = pd.read_csv(os.path.join(RESOURCES_DIR, "Airline_surveys_sample.csv"))
airport_map = pd.read_csv(os.path.join(RESOURCES_DIR, "airport-codes.csv"))
