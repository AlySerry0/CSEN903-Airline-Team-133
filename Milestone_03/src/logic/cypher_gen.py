...
def cypher_flight_performance(entities: dict) -> str:
    ...
    # Check for route information in relations
    airport_dep = entities["relations"].get("departs_from", [{}])[0].get("airport")
    airport_arr = entities["relations"].get("arrives_at", [{}])[0].get("airport")

    q = "MATCH (j:Journey)-[:ON]->(f:Flight)\n"
    
    # Add route filtering if available
    if airport_dep:
        q += f"MATCH (f)-[:DEPARTS_FROM]->(d:Airport {{station_code: '{airport_dep}'}})\n"
    if airport_arr:
        q += f"MATCH (f)-[:ARRIVES_AT]->(a:Airport {{station_code: '{airport_arr}'}})\n"
